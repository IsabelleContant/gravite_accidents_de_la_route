...
st.markdown(""" 
            ...
            .css-7ym5gk {background-color: #135e58; }
            .css-7ym5gk p {color: white; font-weight: bold; font-size:1.5em;}
            .css-7ym5gk:active {border-color: #8B4147; background-color: #8B4147;}
            .css-7ym5gk:focus:not(:active) {border-color: #8B4147; background-color: #8B4147;}
            .css-7ym5gk:hover {border-color: #418B85; background-color: #418B85;}
            </style> """, 
...
with col2:
    # Bouton pour obtenir les prédictions
    run_prediction = st.button("Cliquez pour obtenir les Prédictions")
st.write("")

if run_prediction:
    
    # Encodage du nom de la variable pour l'URL
    ...
    if response.status_code == 200:
        ...
        forecast['ds'] = pd.to_datetime(forecast['ds']) # Convertir la colonne 'ds' en datetime

    else:
        st.write("Erreur lors de la récupération des prédictions de l'API.")
